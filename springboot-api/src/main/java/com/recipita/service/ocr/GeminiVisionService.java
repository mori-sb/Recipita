...
public class GeminiVisionService {
    ...
    // ✅ ObjectMapper をコンストラクタで注入
    public GeminiVisionService(ObjectMapper objectMapper) {
        this.objectMapper = objectMapper;
    }

    public RawReceiptResultDto extractTextFromImage(String base64Image) throws JsonProcessingException {
        ...
        Map<String, Object> textPart = Map.of(
                ...
                以下の画像は日本のレシートです。画像の内容をOCRし、次の情報を日本語でJSON形式で抽出してください。
                   ...
                   1. **店舗名の候補（1〜8文字）**
                      - 店名と思われる行をそのまま抽出してください。住所や支店名が含まれないように抽出して。
                      - 英語の固有名詞の場合は、そのまま出力してください。
                   ...
                   4. 各商品を以下のカテゴリに分類し、カテゴリごとの合計金額をJSON形式で返してください。
                       ...
                       「食費」「日用品」「交通費」「飲み会」「カフェ」「服」「医療費」「娯楽」「その他」
        ...
        if (response.getStatusCode().is2xxSuccessful()) {
            ...
            if (candidates != null && !candidates.isEmpty()) {
                ...
                return objectMapper.readValue(jsonText, RawReceiptResultDto.class);
            }
